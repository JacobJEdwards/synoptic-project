...
export async function searchRecipes(recipes: Recipe[], searchTerm: string) {
    const filteredRecipes = recipes.filter((recipe: Recipe) => recipe.title.includes(searchTerm) || recipe.description?.includes(searchTerm))
